"""
Agent Reputation Scoring -- behavioral trust and reputation scoring for AI agents.

This is the canonical module name for MnemoPay's behavioral reputation-scoring
engine. The legacy `mnemopay.fico` and `mnemopay.agent_credit_score` modules
remain as deprecated aliases for back-compat.

The five components, weights, and tier thresholds are:
...
Usage
-----
    from mnemopay import AgentReputationScoring, AgentReputationInput

    engine = AgentReputationScoring()
...
class AgentReputationScoring:
    """Agent Reputation Scoring engine (300-850).

    Deterministic: same inputs, same score.
    """

    def __init__(self, config: Optional[AgentReputationConfig] = None) -> None:
        self.config = config or AgentReputationConfig()
        ...
        if abs(weight_sum - 1.0) > 0.001:
            raise ValueError(f"AgentReputationScoring weights must sum to 1.0, got {weight_sum:.4f}")
        ...
        if any(w <= 0 for w in [
            ...
            raise ValueError("All AgentReputationScoring weights must be positive")

    def compute(self, inp: AgentReputationInput) -> AgentReputationResult:
        ...
        def _make_component(raw: Dict, weight: float) -> AgentReputationComponent:
            return AgentReputationComponent(
                score=raw["score"],
                weight=weight,
    ...
    def _compute_payment_history(
        self, txs: List[AgentReputationTransaction], now: float,
    ) -> Dict[str, Any]:
        factors: List[str] = []
    ...
    def _compute_credit_utilization(
        self,
        txs: List[AgentReputationTransaction],
        budget_cap: float,
        period_days: int,
        now: float,
    ) -> Dict[str, Any]:
    ...
    def _compute_behavior_diversity(
        self, txs: List[AgentReputationTransaction], memories_count: int,
    ) -> Dict[str, Any]:
        factors: List[str] = []
    ...
    def _validate_input(self, inp: AgentReputationInput) -> None:
        if not isinstance(inp, AgentReputationInput):
            raise ValueError("AgentReputationInput is required")
        if not isinstance(inp.transactions, list):
            raise ValueError("AgentReputationInput.transactions must be a list")
        if not isinstance(inp.created_at, datetime):
            raise ValueError("AgentReputationInput.created_at must be a datetime")
        if not isinstance(inp.fraud_flags, (int, float)) or inp.fraud_flags < 0:
            raise ValueError("AgentReputationInput.fraud_flags must be a non-negative number")
        if not isinstance(inp.dispute_count, (int, float)) or inp.dispute_count < 0:
            raise ValueError("AgentReputationInput.dispute_count must be a non-negative number")
        if not isinstance(inp.disputes_lost, (int, float)) or inp.disputes_lost < 0:
            raise ValueError("AgentReputationInput.disputes_lost must be a non-negative number")
        if inp.disputes_lost > inp.dispute_count:
            raise ValueError("AgentReputationInput.disputes_lost cannot exceed dispute_count")
        if not isinstance(inp.warnings, (int, float)) or inp.warnings < 0:
            raise ValueError("AgentReputationInput.warnings must be a non-negative number")
        if inp.budget_cap is not None and (
            not isinstance(inp.budget_cap, (int, float)) or inp.budget_cap <= 0
        ):
            raise ValueError("AgentReputationInput.budget_cap must be a positive number")
    ...
    @staticmethod
    def serialize(result: AgentReputationResult) -> str:
        """Serialize an AgentReputationResult to JSON."""
